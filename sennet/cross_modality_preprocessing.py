...
def plot_gridwise_scatter(coord1, expr1, coord2, expr2, bins=50, gene1='CDKN1A', gene2='p16',save_root=None):
    # Determine shared spatial bounds
...
for i in range(18,len(sennet_pairs)):
    ...
    warped_xenium_coor = np.stack([
        xenium_gene_data.obs['x_aligned_combined'].values,
        xenium_gene_data.obs['y_aligned_combined'].values
    ], axis=1)

    warped_codex_coor = np.stack([
        codex_gene_data.obs['x_aligned_combined'].values,
        codex_gene_data.obs['y_aligned_combined'].values
    ], axis=1)
    ...
    r,p = plot_gridwise_scatter(
        ...
        bins=20, gene1=gene_key, gene2='p16'
    )
    r_values.append(r)
    p_values.append(p)
