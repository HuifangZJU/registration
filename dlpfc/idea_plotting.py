...
def plot_maps(feature_maps, layer_gap=6.5, grid_color='grey', grid_alpha=0.3):
    ...
    # default = ['Blues', 'Purples','Purples',
    #            'Purples']
    default = ['Blues', 'Blues','Blues',
               'Blues']
    cmap_names = default[:len(feature_maps)]
    ...
    for li, fm in enumerate(feature_maps):
        ...
        cmap = cm.get_cmap(cmap_names[li]).copy()
        cmap.set_bad((1, 1, 1, 0))  # RGBA, alpha=0
        ax.imshow(masked, cmap=cmap, norm=norm,
                  origin='lower', extent=[x0, x0 + cols, 0, rows])

        rows_i, cols_i = np.where(fm > 0)  # indices of non-zeros
        if rows_i.size == 0:  # skip empty slabs
            continue

        xs = x0 + cols_i + 0.5  # centre of each tile
        ys = rows_i + 0.5

        mstyle = ['x', 'o', '^', '_'][li % 4]  # cycle through symbols

        if mstyle == 'x' :  # special-case ‘x’
            ax.scatter(xs, ys,
                       marker='x',
                       c='k',  # colour for line marker
                       s=120,  # size: tweak until visible
                       linewidths=1.5)  # thickness of the strokes
        elif mstyle == '^':  # triangle: hollow
            ax.scatter(xs, ys,
                       marker='^',
                       facecolors='none',
                       edgecolors='k',
            ...
            ax.scatter(xs, ys,
                       marker='_',  # vertical short line
                       s=100,  # adjust length (area)
                       color='k',  # black
                       linewidths=1.8)  # stroke thickness
        else:  # 'o' or 's'
            ax.scatter(xs, ys,
                       marker=mstyle,
                       facecolors='none',  # hollow look
                       edgecolors='k',
                       linewidths=1.2,
                       s=140)
        # ---- bold black frame ---------------------------------------
...
def plot_maps2(feature_maps, layer_gap=6.5, grid_color='grey', grid_alpha=0.3):
    ...
    for li, fm in enumerate(feature_maps):
        ...
        # ---- Draw dashed-line boxes around non-zero cells---------------------------------------
        if li==0:
            rows_i_2, cols_i_2 = np.where(feature_maps[1] > 0)
            for xi, yi in zip(cols_i_2, rows_i_2):
                rect = patches.Rectangle(
                    (x0 + xi, yi),  # lower-left corner of cell in the slab
                    width=1,
                    height=1,
                    fill=False,
                    edgecolor='gray',
...
def get_grid(coords, step=1, lower=0.2, upper=0.8,seed=None):
    # ── 1️⃣  original grid ─────────────────────────────────────────
    xmin=0
    ymin=0
    xmax=5
...
def plot_imputation_grids(nx, ny, grid):
    ...
    imputed_values[imputed] = np.random.uniform(0.4, 0.7, imputed.sum())
    ...
    cmap = cm.viridis.copy()
    cmap.set_bad('white')
    ax.imshow(mask, origin='lower', cmap=cm.Blues, vmin=0.3, vmax=0.9)

    rows_i, cols_i = np.where(mask > 0)  # indices of non-zeros
    xs = cols_i   # centre of each tile
    ys = rows_i
    ax.scatter(xs, ys,
               marker='x',
               c='k',  # colour for line marker
               s=220,  # size: tweak until visible
               linewidths=2.5)  # thickness of the strokes
    ...
    #           origin='lower', cmap=cm.Reds, vmin=0.3, vmax=1.5, alpha=0.25)

    # rows_i_2, cols_i_2 = np.where(imputed_values> 0)
    # for xi, yi in zip(cols_i_2, rows_i_2):
    #     rect = patches.Rectangle(
    #         (xi-0.5, yi-0.5),  # lower-left corner of cell in the slab
    #         width=1,
    #         height=1,
    #         fill=False,
    #         edgecolor='gray',
    ...
    # light grid lines
    for g in range(nx +1): ax.axvline(g - 0.5, color='grey', lw=.4, linestyle='dashed')
    for g in range(ny +1): ax.axhline(g - 0.5, color='grey', lw=.4, linestyle='dashed')
    ...
    head_gap = 0.15  # leave 12 % of the cell size before empty-centre
    for gy in range(ny):
        for gx in range(nx):
            if grid[gy, gx] == 0:  # empty bin
                ...
                for dx, dy in dirs:
                    ...
                    if 0 <= ngx < nx and 0 <= ngy < ny and grid[ngy, ngx] > 0:
                        # sx, sy = ngx, ngy  # start at neighbour centre
                        sx, sy = gx + 0.5*dx, gy + 0.5*dy  # start at neighbour centre
...
def plot_overlay(feature_maps,grid_color='grey', grid_alpha=0.3):
    rows, cols = feature_maps[0].shape
    ...
    for fm, cmap_name in zip(feature_maps, cmap_names):
        masked = np.ma.masked_equal(fm, 0)  # 0-cells → mask

        # copy cmap and make masked values fully transparent
        ...
        ax.imshow(masked,
                  origin='lower',
                  cmap=cmap,
                  vmin=0, vmax=1,  # scale 0‒1 if data already in that range
    ...
    for li, fm in enumerate(feature_maps):
        ...
        xs = cols_i + 0.5  # centre of each tile
        ys = rows_i + 0.5
        ...
        #     (1, 1),  # lower-left corner of slab
        #     width=cols-1 ,  # slab width  (x-direction)
        #     height=rows -1,  # slab height (y-direction)
        #     fill=False,  # outline only
        #     edgecolor='black',
        #     linewidth=1.0  # “boldness”; raise for thicker lines
        # )
...
def plot_overlay2(feature_maps,grid_color='grey', grid_alpha=0.3):
    overlay_map = np.zeros((7, 7), dtype=feature_maps[0].dtype)
    overlay_map[1:, 1:] = feature_maps[0]*1.3
    # overlay_map = feature_maps[0]*1.3


    rows, cols = overlay_map.shape
    ...
    fig, ax = plt.subplots(figsize=(5, 5))
    # --------- draw each map side-by-side ------------------------
    masked = np.ma.masked_equal(overlay_map, 0)

    ax.imshow(masked, cmap=cmap, norm=norm,
              origin='lower', extent=[0, cols, 0, rows])

    rows_i, cols_i = np.where(overlay_map > 0)  # indices of non-zeros


    for li, _ in enumerate(feature_maps):
        mstyle = ['x', 'o', '^', '_'][li % 4]  # cycle through symbols
        if mstyle == 'x':  # special-case ‘x’
            xs = cols_i + 0.25  # centre of each tile
            ys = rows_i + 0.25
            ax.scatter(xs, ys,
                       marker='x',
                       c='k',  # colour for line marker
            ...
            xs = cols_i + 0.75  # centre of each tile
            ys = rows_i + 0.25
            ax.scatter(xs, ys,
                           marker='_',  # vertical short line
                           s=100,  # adjust length (area)
            ...
            ax.scatter(xs, ys,
                       ...
                       s=140)
        # ---- bold black frame ---------------------------------------
        # List of grayscale colors from dark to light
        gray_shades = ['0.1', '0.3', '0.5', '0.7']

        # Rectangle positions
        positions = [(0, 0), (1, 1), (1, 0), (0, 1)]

        # Add rectangles with different gray levels
...
nx, ny, _, _, _, grid = get_grid(coor1)
_, _, _, _, _, grid0 = get_grid(coor2)
_, _, _, _, _, grid1 = get_grid(coor3)
_, _, _, _, _, grid2 = get_grid(coor4)
a,b, c, d,e,  grid_plus = get_grid(coor1,lower=0.6,upper=0.9)

plot_imputation_grids(nx,ny,grid)
maps = [grid,grid2]
# maps = [grid,grid0,grid1]
