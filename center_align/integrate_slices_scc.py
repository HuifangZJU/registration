...
def compare_original_vs_fused(adata_original, adata_fused, save_suffix=None, n_top_genes=2000, n_pcs=30, n_neighbors=15):
    ...
    g, l = generate_graph_from_labels_gpu(adata_original, adata_original.obs["original_clusters"])
    single_score = np.abs(spatial_coherence_score_fast(g, l))
    n_clusters = adata_original.obs["original_clusters"].nunique()
    ...
    axes[0].scatter(
        ...
        c=adata_original.obs["original_clusters"].astype("category").cat.codes,
        cmap="tab20", s=20
    )
...
for method in ['ours1','nicetrans','initial','paste','SANTO','simpleitk','voxelmorph']:
    ...
    for i in [2]:
        ...
        # plot_stl_score(adata_fused)

        # compare_original_vs_fused(adata_single, adata_fused, f"patient{i}_{method}")
