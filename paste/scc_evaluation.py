...
def visualize_centroid_shift(region_masks_A, region_masks_B,region_mean_shift,save_suffix,save=True):
    ...
    for idx, class_label in enumerate(region_masks_A.keys()):
        ...
        plt.scatter(coords_A[:, 0], coords_A[:, 1], color=class_color, marker="o",  s=20, alpha=0.5)
        plt.scatter(coords_B[:, 0], coords_B[:, 1], color=class_color,  marker="x",s=20, alpha=0.5)

        # Plot centroids
        plt.scatter(centroids_A[0], centroids_A[1], edgecolor='black', color=class_color, marker="o", s=100,
                    label=f"Layer {class_label} - Centroid A")
        plt.scatter(centroids_B[0], centroids_B[1], edgecolor='black', color=class_color, marker="s", s=100,
                    label=f"Layer {class_label} - Centroid B")
...
def visualize_dice_regions(region_masks_A, region_masks_B, dice_scores, average_dice_score,save_suffix,save,alpha=0.15):
    ...
    for idx, class_label in enumerate(region_masks_A.keys()):
        ...
        plt.scatter(region_coords_A[:, 0], region_coords_A[:, 1], color=color, alpha=0.3, s=20, label=f"Region {class_label} (A)")
        plt.scatter(region_coords_B[:, 0], region_coords_B[:, 1], color=color, alpha=0.3, s=20, marker="x", label=f"Region {class_label} (B)")
...
def compute_region_dice_score(coords_A, labels_A, coords_B, labels_B, save_suffix,mask_shape=(2048, 2048), alpha=0.05,visualize=True,save=True):
    """
...
def evaluate_single_pair(folder_path,i,j,suffix):
    ...
    results = {
        "Class-wise Dice Coefficient": compute_region_dice_score(coords_A, labels_A, coords_B, labels_B,figure_suffix,visualize=True,save=False),
        "Spatial Cross-Correlation": spatial_cross_correlation(coords_A, labels_A, coords_B, labels_B, visualize=False),
        "Mean Centroid Shift": mean_centroid_shift(coords_A, labels_A, coords_B, labels_B,figure_suffix,visualize=True,save=False),
...
average_results = evaluate_multiple_pairs("/home/huifang/workspace/code/registration/result/nicetrans/SCC/",keys,"nicetrans")
