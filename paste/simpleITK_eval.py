...
def get_simpleITK_transformation(slice1, slice2, shrink_factor=4):
    # Read original-size images
    fixed_image_full = sitk.ReadImage(slice1.image_path, sitk.sitkFloat32)
    moving_image_full = sitk.ReadImage(slice2.image_path, sitk.sitkFloat32)

    # 1) Downsample (shrink) both images for the registration
    #    (You could also use sitk.Resample if you want a specific output size/spacing.)
    fixed_image = sitk.Shrink(fixed_image_full, [shrink_factor, shrink_factor])
    moving_image = sitk.Shrink(moving_image_full, [shrink_factor, shrink_factor])
    ...
    displacement_filter.SetReferenceImage(fixed_image_full)  # <-- reference is the full-size fixed image
    ...
    resampler.SetReferenceImage(fixed_image_full)  # use the full-size fixed as reference
    resampler.SetTransform(itk_transform)
    resampler.SetInterpolator(sitk.sitkLinear)
    registered_image = resampler.Execute(moving_image_full)

    # Convert images to numpy arrays, if needed
    fixed_array = sitk.GetArrayViewFromImage(fixed_image_full)
    moving_array = sitk.GetArrayViewFromImage(moving_image_full)
...
# sample_groups = [["151507", "151508"],[ "151669", "151670"]]
...
for j in range(len(layer_groups)):
    ...
    for i in range(len(slices) - 1):
        ...
        print('calculating simpleITK transformation ... ')
        displacement_field,fixed_image,moving_image, warped_image = get_simpleITK_transformation(slices[i], slices[i+1])
        print('calculation is done, perform transformations..')


        scale_path = slices[i+1].image_scale_path
        with open(scale_path, 'r') as f:
            data = json.load(f)
            scale = data['tissue_hires_scalef']
        fixed_pts = scale * slices[i].image_coor
        moving_pts = scale * slices[i+1].image_coor
        # warped_pts = moving_pts
        warped_pts = transform_uv_with_displacement(moving_pts, displacement_field)

        fixed_label = np.matrix(slices[i].obs['layer_guess_reordered'].map(mapping_dict))
        moving_label = np.matrix(slices[i + 1].obs['layer_guess_reordered'].map(mapping_dict))
        # fixed_label = np.load("/home/huifang/workspace/code/registration/data/DLPFC/huifang/"+str(j)+"_"+str(i)+"_region_label.npy")
        # moving_label = np.load("/home/huifang/workspace/code/registration/data/DLPFC/huifang/" + str(j) + "_" + str(
        #     i+1) + "_region_label.npy")

        # np.savez("../result/simpleITK/DLPFC//" + str(j) + "_" + str(i) + "_result", pts1=fixed_pts,pts2=warped_pts,img1=fixed_image,img2=warped_image,label1=fixed_label,label2=moving_label)
        np.savez("../result/original/DLPFC/" + str(j) + "_" + str(i) + "_result", pts1=fixed_pts, pts2=moving_pts,
                 img1=fixed_image, img2=moving_image, label1=fixed_label, label2=moving_label)
        ...
        if visualization:
            ...
            a[0].scatter(fixed_pts[:, 0], fixed_pts[:, 1], color=colors1, s=5,
                            label="Slice 1")
            ...
            a[1].scatter(moving_pts[:, 0], moving_pts[:, 1],color=colors2, s=5,
                            label="Slice 2")
            ...
            a[2].scatter(warped_pts[:, 0], warped_pts[:, 1], color=colors2, s=5,
            ...
            a[0, 0].scatter(fixed_pts[:, 0], fixed_pts[:, 1], color=colors1,label="Slice 1", alpha=0.6)
            a[0, 0].scatter(moving_pts[:, 0], moving_pts[:, 1], color=colors2,label="Slice 2", alpha=0.6)
            ...
            a[0, 1].scatter(fixed_pts[:, 0], fixed_pts[:, 1],color=colors1, label="Warped Slice 1", alpha=0.6)
            a[0, 1].scatter(warped_pts[:, 0], warped_pts[:, 1],color=colors2, label="Warped Slice 2", alpha=0.6)
            a[0, 1].invert_yaxis()
