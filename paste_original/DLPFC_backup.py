...
def get_svg(sl):
    # 2) Build the spatial neighbour graph  ──────────────────────
    sq.gr.spatial_neighbors(
        sl,
        coord_type="generic",  # or "grid" for regular 10x layout
        n_neighs=50,  # k‐NN; use `n_rings=` for Visium rings
    )

    # sq.gr.spatial_smooth(sl, output_layer="smooth", n_neighs=50, kernel="gaussian", sigma=0.5)

    # 3) Run the global spatial-autocorrelation test (Moran’s I) ─
    ...
    moran = sl.uns["moranI"]  # pandas DataFrame
    # pval_cols = [c for c in moran.columns if c.endswith('_fdr_bh')]
    # sig = moran.loc[(moran[pval_cols] < 0.1).any(axis=1)]
    sig = (
        moran
        .query("(I > 0.01) & (pval_norm_fdr_bh < 0.2)")  # whatever threshold you like
        .sort_values(by=["I", "pval_norm_fdr_bh"],  # I ↓, p-value ↑
                     ascending=[False, True])  # big I, small p first
    )

    top_genes = sig.head(64).index.tolist()
    return top_genes
...
for k,slices in enumerate(layer_groups):
    ...
    for sl in slices:
        ...
        hvg_slices.append(sl)

        # gene_sets = get_svg(sl)
        # sl = sl[:, gene_sets]
    ...
    #     a[i].imshow(img)
    #     a[i].scatter(coors[:, 0], coors[:, 1],s=5)
    # plt.show()
    # #
    #
