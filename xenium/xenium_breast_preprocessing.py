...
def get_xenium_data():
    ...
    image_paths = [["Xenium_V1_FFPE_Human_Breast_ILC","Xenium_V1_FFPE_Human_Breast_ILC_With_Addon"]]
    groups = []
...
for k,slices in enumerate(groups):
    ...
    for i, sl in enumerate(slices):
        ...
        # labels = sl.obs["cluster_id"].astype("category").cat.codes.to_numpy()
        ...
        resized_image,resized_coords = resize_and_flip_image_coords(image,coords,image_size)
    ...
    for i, data_slice in enumerate(gene_data_list):
        ...
        # plt.imshow(image)
        # # plt.scatter(coords[:,0],coords[:,1],c=labels,s=1, cmap="tab20")
        # plt.show()
        ...
        # imageio.imwrite(
        #     f"/media/huifang/data/registration/xenium/breast/{k}_{i}_image_{image_size}.png",
        #     image.astype(np.uint8)  # scale to 8-bit if image is 0–1 float
        # )
        # np.save("/media/huifang/data/registration/xenium/breast/" + str(k) + "_" + str(i) + f"_pca_out_{image_size}.npy", feature_matrix)
        # np.save("/media/huifang/data/registration/xenium/breast/" + str(k) + "_" + str(i) + f"_pca_mask_{image_size}.npy", gene_mask)
        # np.savez("/media/huifang/data/registration/xenium/breast/" + str(k) + "_" + str(i) + f"_validation_{image_size}", coord=coords,label = labels)
        np.savez(
            "/media/huifang/data/registration/breast/" + str(k) + "_" + str(i) + "_marker_label", label=labels)
