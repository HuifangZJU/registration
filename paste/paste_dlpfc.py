...
for id,pid in zip(sample_list,processed_id):
    adatas[id].image_path = '/mnt/data/registration/DLPFC/huifang/{0}_image_512.png'.format(pid)
    info = np.load('/mnt/data/registration/DLPFC/huifang/{0}_validation.npz'.format(pid))
...
#        slice0 = layer_groups[j][i]
#        slice1 = layer_groups[j][i+1]
#        pi0 = pst.match_spots_using_spatial_heuristic(slice0.obsm['spatial'],slice1.obsm['spatial'],use_ot=True)
#        start = time.time()
#        pis[j][i] = pst.pairwise_align(slice0, slice1,alpha=alpha,G_init=pi0,norm=True,verbose=False,backend=ot.backend.TorchBackend(),use_gpu=True)
#        tt = time.time()-start
#        acc = mapping_accuracy(slice0.obs['layer_guess_reordered'],slice1.obs['layer_guess_reordered'],pis[j][i])
#        print(j,i,'Accuracy',acc,'time',tt)
#        np.savetxt('../data/DLPFC/saved_results/transformed_pairwise_init_{0}_{1}_{2}.gz'.format(j,i,'ot'), pis[j][i], delimiter=',')
...
for j in range(len(layer_groups)):
    for i in range(len(layer_groups[j])-1):
        pis[j][i]=np.loadtxt('../data/DLPFC/saved_results/transformed_pairwise_init_{0}_{1}_{2}.gz'.format(j,i,'ot'), delimiter=',')
...
for j in range(len(layer_groups)):
    ...
    for i in range(len(slices)-1):
        ...
        labels1 = np.matrix(slice1.obs['layer_guess_reordered'].map(mapping_dict))
        labels2 = np.matrix(slice2.obs['layer_guess_reordered'].map(mapping_dict))
        ...
        np.savez("/mnt/data/registration/result/pairwise_align/DLPFC/paste/" + str(j) + "_" + str(i) + "_result", pts1=adjusted_warped_slice1,pts2=adjusted_warped_slice2,img1=warped_img1,img2=warped_img2,label1=labels1,label2=labels2)
