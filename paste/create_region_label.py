...
root_folder = "/home/huifang/workspace/code/registration/result/original/DLPFC/"
for i in range(1,3):
    for j in [0,2]:
        data_path = root_folder+str(i)+"_"+str(j)+"_result.npz"
        img_fixed, img_moving, coords_A, coords_B, labels_A, labels_B = load_data_from_folder(data_path)
        coords = coords_B
        labels = labels_B
        regions_A = get_connected_regions(coords, labels)
        labels = combine_disconnected_regions(regions_A,len(labels))

        # labels = switch_label_ids(labels, 1, 4)
        # labels = switch_label_ids(labels, 2, 3)
        # labels = switch_label_ids(labels, 3, 4)

        # np.save("../data/DLPFC/huifang/" + str(0) + "_" + str(3) + "_region_label.npy", labels)
