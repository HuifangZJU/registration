...
def get_mouse_data(
    ...
    for k in [0,1]:
        adatas=[]
        for slice_id in ['v1','v2']:
            adata = sc.read_h5ad("/media/huifang/data/registration/mouse/huifang/h5ad/" + f"group_{k}_slice_{slice_id}_clusters_smoothed.h5ad")
            adatas.append(adata)
        layer_groups.append(adatas)
    return layer_groups
...
def show_clusters(img,coors,labels):
...
crop_paras = [[(0,200,1800,1024),(50,100,1800,1024)],
              [(160,100,1800,1024),(160,200,1800,1024)]]
image_size = 512
...
for k,slices in enumerate(layer_groups):
    ...
    for i, sl in enumerate(slices):
        ...
        gene_data_list.append(sl.obsm['X_pca'][:,:10])
        ...
        labels = list(sl.obs['clusters_expr_smoothed'].astype(int))
        labels = np.asarray(labels)

        # show_clusters(cropped_image,cropped_coor, labels)
    ...
    reduced_data = np.vstack(gene_data_list)
